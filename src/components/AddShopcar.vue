                      ...
                      <el-row>
                        ...
                        <el-col :span="24">
                          <el-form-item label="用户">
                            <input maxlength="-1" type="hidden" name="user" id="user" v-model="shopcar.user">
                            <el-select name="userid" id="userid" v-on:change="userchange()" v-model="shopcar.userid"
                                       placeholder="请选择用户">
                              <el-option
    ...
    methods: {
      ...
      searchuser() {
        ...
        this.$axios.post("/searchuserjson", "")
          .then(res => {
            _this.userall = res.data.userall;
            var userid = _this.$route.query.userid;
            if (userid != null) {
              _this.userall = _this.userall.filter(function (item) {
                return item.id == userid;
              })
            }
            ;
            _this.shopcar.userid = _this.userall[0]["id"];
            _this.shopcar.user = _this.userall[0]["name"];
          })
          .catch(err => {
            console.log(err)
          })
      ...
      userchange: function () {
        let _this = this;
        _this.shopcar.user = _this.userall.filter(function (item) {
          return item.id == _this.shopcar.userid;
        })[0]["name"];
        _this.$forceUpdate();
      },
      addshopcaract: function () {
        ...
        this.$axios.post("/addshopcaractjson", _this.shopcar)
          .then(res => {
            ...
            _this.$router.push('/ShopcarManage');
          })
          .catch(err => {
            console.log(err)
            _this.$message({
